  ...
  <div class="grid grid-cols-3 gap-1">
    ...
    >
      ...
      <div class="w-5/6 mx-auto " v-for="todo in todoList" :key="todo.id">
        <div id="border" class="flex items-center justify-between my-2 rounded-lg">
          <div class="flex">
            <input
              type="checkbox"
              @click="changeComplete(todo)"
              :checked="todo.isCompleted"
              class="w-8 h-8 mx-2 mt-1 rounded-lg cursor-pointer bg-amber-200 hover:bg-amber-400 border-3 border-rose-500 checked:bg-green-500"
            />
            <p>{{ todo.content }}</p>
          </div>
