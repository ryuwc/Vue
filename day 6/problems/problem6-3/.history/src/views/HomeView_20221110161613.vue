<template>
  <div class="grid grid-cols-3 gap-1">
    <nav
      class="flex flex-col col-span-1 mx-1 my-4 text-2xl border-2 border-black border-solid rounded-lg h-72"
    >
      <router-link to="/" class="py-1 pt-8">모든</router-link>
      <router-link to="/about" class="py-1">오늘</router-link>
      <router-link to="/important" class="py-1">중요</router-link>
    </nav>
    <div
      class="col-span-2 my-4 border-2 border-black border-solid rounded-lg h-72"
    >
      <h1>모든 할일</h1>
      <div class="block">
      </div>
      <hr class="w-5/6" />
      <div class="flex w-5/6 mx-auto" v-for="todo in todoList" :key="todo.id">
        <div class="flex items-center">
          <input
            type="checkbox"
            :checked="todo.isCompleted"
            class="w-8 h-8 rounded-lg cursor-pointer bg-amber-200 hover:bg-amber-400 border-3 border-rose-500 checked:bg-green-500"
          />
          <p style="margin-left: auto;" class="ml-auto">{{ todo.content }}</p>
          <div v-if="todo.isImportant" class="">
            <i style="color: goldenrod;" class="ml-auto fa-solid fa-star"></i>
          </div>
          <div v-else class="ml-auto">
            <i style="color: goldenrod;" class="pl-auto fa-regular fa-star"></i>
          </div>
        </div>
      </div>
    </div>
